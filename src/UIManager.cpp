...
void UIManager::render(){
    ...
    if (placingTowerType != TowerType::None){
        ...
        std::vector<sf::Vector2f> validPlots = {
    ...
    if (placingTowerType != TowerType::None){
        ...
        for (const auto& plot: validPlots){
            if (length(mousePos - plot) < 40.0f) {// 80 pxls diameter clickable area
                // Deduct gold depending on tower type
                if (placingTowerType == TowerType::Archer) currentGold -= 50;
                else if (placingTowerType == TowerType::Cannon) currentGold -= 100;
                else if (placingTowerType == TowerType::Magic) currentGold -= 120;

                updateGold(currentGold); // update text on screen
                placingTowerType = TowerType::None; // reset
                archerButton.setFillColor(sf::Color(70, 70, 70));  // normal color
                cannonButton.setFillColor(sf::Color(70,70,70));
                magicButton.setFillColor(sf::Color(70,70,70));
                cout << "Tower placed at (" << plot.x << "," << plot.y << ")!\n";
                return;
            }
        }
